...
ggcorr(data[, c("Price", "Mileage", "Cylinder", "Liter", "Cruise")],
...
base_model <- lm(Price ~ Mileage + Cylinder + Liter + Cruise, data = data)

#-----c. Вывести результаты анализа базовой модели-----
summary(base_model)
#Остатки показывают, насколько предсказанные значения Price отличаются от реальных.
...
#Price = 2799.86−0.164*Mileage+3007.80*Cylinder+455.73*Liter+6076.05*Cruise

#-----e.	Проверить значимость каждого отдельного коэффициента с помощью  T-test-----
#из summary():
#Mileage: коэффициент отрицательный (-0.164), значим (***), чем больше пробег, тем ниже цена.
#Cylinder: положительный (3007.8), значим (***), больше цилиндров -> выше цена.
#Liter: коэффициент положительный, но не значим (p = 0.597).
#Cruise: сильная положительная связь, значим (***).
...
ggplot(data, aes(x = Predicted, y = Price)) +
  ...
  labs(title = "Фактические vs Предсказанные значения Price",
       x = "Predicted Price",
       y = "Actual Price") +
  theme_minimal()
...
#Price=2332.52−0.164*Mileage+3350.15*Cylinder+6126.56*Cruise
...
data$Price_tf <- (data$Price^(lambda_opt) - 1)/(lambda_opt)

# Новая линейная модель
boxcox_model <- lm(Price_tf ~ Mileage + Cylinder + Cruise, data = data)

#-----s.	В случае получения новой модели в предыдущем пункте проанализировать новую модель.-----
summary(base_model) #c
#Multiple R-squared = 0.4688 -> модель объясняет ~ 47% дисперсии Price_tf (на 7% лучше).
#Price_tf=1.592−1.487e-8*Mileage+2.661e−4*Cylinder+6.762e−4*Cruise (d)
#Все коэффициенты значимы (p < 0.001), даже Mileage, который ранее был менее значимым.(e)
#F = 235.3, p-value < 2.2e-16 -> модель значимо объясняет зависимую переменную.(f)
data$Predicted_tf <- predict(boxcox_model)

ggplot(data, aes(x = Predicted_tf, y = Price_tf)) +
  geom_point(color = "darkblue") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Фактические vs Предсказанные значения Price_tf (Box-Cox)",
       x = "Predicted Price_tf",
       y = "Actual Price_tf") +
  theme_minimal() #g
